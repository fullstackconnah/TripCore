import { useEventTemplates, useActivities } from '@/api/hooks'
import { useState } from 'react'

export default function SettingsPage() {
  const [tab, setTab] = useState<'templates' | 'activities'>('templates')
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        ...
        <p className="text-sm text-[var(--color-muted-foreground)] mt-1">Manage event templates and activity library</p>
      </div>

      <div className="flex gap-4 border-b border-[var(--color-border)]">
        {[{ key: 'templates' as const, label: 'Event Templates' }, { key: 'activities' as const, label: 'Activity Library' }].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`px-4 py-3 text-sm font-medium border-b-2 transition-colors ${tab === t.key ? 'border-[var(--color-primary)] text-[var(--color-primary)]' : 'border-transparent text-[var(--color-muted-foreground)]'}`}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          {templates.map((t: any) => (
            <div key={t.id} className="bg-[var(--color-card)] rounded-xl border border-[var(--color-border)] p-5">
              <div className="flex items-start justify-between mb-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded-full ${t.isActive ? 'badge-confirmed' : 'badge-cancelled'}`}>{t.isActive ? 'Active' : 'Inactive'}</span>
              </div>
            ...
            <tbody className="divide-y divide-[var(--color-border)]">
              {activities.map((a: any) => (
                <tr key={a.id} className="hover:bg-[var(--color-accent)]/50 transition-colors">
                  ...
                  <td className="p-3"><span className={`text-xs px-2 py-0.5 rounded-full ${a.isActive ? 'badge-confirmed' : 'badge-cancelled'}`}>{a.isActive ? 'Active' : 'Inactive'}</span></td>
                </tr>
              ))}
            </tbody>
          </table>
